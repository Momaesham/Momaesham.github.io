...
3. Устанавливаем зависимост ```npm install```
4. Запуск процесса сборки ```npm run dev```
...
Для подлкючения этого модуля на вашу страничку, подключаете в файле ```index.html``` с помощью команды ```{% include "_html/_base/_includes/_doctors.html" %}```

## Для работы с SCSS

Модули (компоненты) для SCSS создаются по пути ```src/assets/css/```. Все модули созданные с помощью HTML теперь требуется стилизировать. Также, модулями. В моём примере это будет ```_doctors.scss```.
