...
1. Клонируем репозиторий ```git clone https://github.com/Momaesham/web-builder.git```
...
«{»% include```_html/_base/_includes/_doctors.html``` %«}» НЕ КОПИРУЙТЕ ЭТУ СТРОЧКУ
...
В папке ```fonts``` находятся необходимые шрифты.
